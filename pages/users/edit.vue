...
b-form(v-if="form")
  b-row(align-h="between")
    b-col
      h5 Editar usuário
      small Editar informações de usuário
      br
      p {{ form }}
  ...
  b-row(align-h="end")
    b-col(cols="auto")
      button.btn.btn-primary.mt-5(type='button' @click.prevent.stop='attUser()')
        span.spinner-border.spinner-border-sm(v-if="usersLoading === 'attUser'" role='status' aria-hidden='true')
        | &nbsp;&nbsp;{{ usersLoading === 'attUser' ? 'Carregando' : 'Salvar alterações' }}
...
export default {
  ...
  mounted () {
    this.form = { ...this.usersViewUser.doc }
  },
  methods: {
    async attUser () {
      await this.attDocumentUser(this.usersViewUser.uid, this.form)
    }
  }
}
</script>
